...
class ProfileController extends ControllerBase
{
    public function registerAction()
    {
        if ($this->request->isPost()) {
            ...
            if ($admin->create() == false) {
                ...
                return $this->response->redirect('404/perfil/registro');
            }
        ...
        if ($this->request->isPost()) {
            ...
            if ($remaining_emails == 0) {
                ...
                if ($block_emails_until) {
                    ...
                    $this->flash->error($this->translate('retry_after', ['date' => $df]));
                    return $this->response->redirect('perfil/actualizar-correo');
                } else {
        ...
        if ($this->request->isPost()) {
            ...
            if ($remaining_emails == 0) {
                ...
                if ($block_emails_until) {
                    ...
                    $this->flash->error($this->translate('retry-after', ['date' => $df]));
                    return $this->response->redirect('perfil/actualizar-correo');
                } else {
                    $admin->block_emails_until = date('Y-m-d H:i:s', strtotime('+30 minutes'));
            ...
            if ($remaining_emails == 0) {
                ...
                if ($block_emails_until) {
                    if ($now > $block_until) {
                        ...
                        if ($admin->update() == false) {
                            $this->flash->error($this->translate('operation_not_updated'));
                            return $this->response->redirect('404/perfil/recuperar');
                        }
                    }
    ...
    public function finishRecoverAction()
    {
        ...
        if ($this->request->isPost()) {
            if ($this->security->checkToken() == false) {
                $this->flash->error($this->translate('invalid-token'));
                return $this->response->redirect($this->request->getHTTPReferer());
            }
