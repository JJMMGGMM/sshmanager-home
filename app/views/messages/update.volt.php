...
{% block title %}{{ tr('update_message') | upper }} {{ message.message_id }}{% endblock %}
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-10 col-md-offset-1 animate-box">
          <h2>
            <strong>{{ tr('update_message') }}</strong>
            <br>
            <a href="{{ url('mensajes') }}" class="btn-warning btn-xs">
              <i class="fa fa-arrow-left"></i> {{ tr('form_back') }}
            </a>
          </h2>
          {{ form('mensajes/actualizar/' ~ message.message_id, 'method': 'post', 'id': 'frm') }}
            <div class="row form-group">
              <div class="col-md-8">
                <label for="identificador">{{ tr('message_identifier') }}</label>
                <input type="text" name="identificador" value="{{ old.identificador is defined ? old.identificador : message.identifier }}" class="form-control frm-id">
              </div>
            </div>
