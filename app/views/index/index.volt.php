...
{% block title %}{{ tr('update_index') | upper }}{% endblock %}
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-10 col-md-offset-1 animate-box">
          <h2>
            <strong>{{ tr('select_section') }}: {{ tr(md_section) }}</strong>
            <br>
            <a href="{{ url('inicio/reiniciar') }}" class="btn-danger btn-xs">
              {{ tr('form_restart_page') }}
            </a>
          </h2>
          {{ form('inicio/elegir-seccion', 'method': 'post', 'id': 'frm_filter') }}
            <div class="row form-group">
              <div class="col-md-12">
                <label for="seccion">{{ tr('select_section') }}</label>
                <select class="form-control" name="seccion">
                  <option value="intro" {{ md_section == 'intro' ? 'selected' : '' }}>
                    {{ tr('edit_head') }}
                  </option>
                  <option value="contenido" {{ md_section == 'contenido' ? 'selected' : '' }}>
                    {{ tr('edit_body') }}
                  </option>
                  <option value="final" {{ md_section == 'final' ? 'selected' : '' }}>
                    {{ tr('edit_foot') }}
                  </option>
                </select>
          ...
          {{ end_form() }}
          {{ form('inicio/previsualizar', 'method': 'post', 'id': 'frm') }}
            <input type="hidden" name="seccion" value="{{ md_section }}">
            <div class="row form-group">
              <div class="col-md-12">
                <label for="contenido">{{ tr('write_content') }}</label>
...
{% block js_custom %}
  ...
  <script type="text/javascript">
    (function() {
      ...
      let frm_filter = {
        frm_id: 'frm_filter',
        filter_btn_id: 'btn_filter',
        filter_btn_class: 'btn-filter',
      }

      let btn_filter_frm;

      btn_filter_frm = document.getElementsByClassName(frm_filter.filter_btn_class)[0];
      btn_filter_frm.setAttribute('id', frm_filter.filter_btn_id);
      ...
      // submit filter
      function disableFilterFrm(filter_frm_id) {
        let form = document.getElementById(filter_frm_id);
      ...
      // filter data
      btn_filter_frm.addEventListener('click', function prepareData(event) {
        setTimeout(function wait() {
          disableFilterFrm(frm_filter.frm_id);
          disableFilterFrm(frm_options.frm_id);
        }, 0);
      });
